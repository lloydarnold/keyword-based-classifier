...
const keyWordList = array_from_file("keywords.txt");
const expectedAnswers = array_from_file("correct answers.txt");

train(10, 5)

function train(numOfEpochs, numOfFiles) {
    /* numOfEpochs is number of times to run through file set
    numOfFiles is number of sample files in the exampleEmails directory
    files in exampleEmails should have name {integer}.txt, and there should be a corresponding expected output for each file in the 
    correct answers.txt file*/
    
    for (i = 0; i < numOfEpochs; i ++){
        for (j = 1; j <= numOfFiles; j ++){
            sampleFile = fs.readFileSync(`exampleEmails/${j}.txt`, "utf8")
            ...
            weights = updateWeights(scores, keyWordVector, keyWordList, j - 1)
            save_weights()
        }
    };
};

function updateWeights(scores, keyWordVector, keyWordList, j) {
      weights = changeWeights(correctDecision(getActualDecision(j), scores), indexOfMax(scores), keyWordVector, keyWordList)
return weights
};
...
function getActualDecision(j) {
    return expectedAnswers[j]
};
...
function array_from_file(relativeFilePath) {
    // reads in file and returns array of lines
    let file = fs.readFileSync(relativeFilePath, "utf8")
